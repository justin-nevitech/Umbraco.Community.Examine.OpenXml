...
[![GitHub license](https://img.shields.io/github/license/justin-nevitech/Umbraco.Community.Examine.OpenXml?color=8AB803)](../LICENSE)
...
The index stores the following fields for each document:

| Field | Description |
|---|---|
| `nodeName` | The name of the media item |
| `id` | The media item ID |
| `path` | The content tree path |
| `fileTextContent` | The extracted text content from the document |
...
The package auto-registers via an Umbraco composer, so no additional startup configuration is needed. The `OpenXmlIndex` will be available immediately after restarting the site.

## Searching the Index

You can query the `OpenXmlIndex` from any Razor view or controller using Examine's `IExamineManager`. Here is an example Razor view that searches the index for text content:
